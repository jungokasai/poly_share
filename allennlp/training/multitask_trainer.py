...
class MultitaskTrainer(Registrable):
    ...
    def __init__(self,
                 ...
                 validation_dataset_aux: Optional[Iterable[Instance]] = None,
                 patience: Optional[int] = None,
                 validation_metric: str = "-loss",
                 validation_iterator: DataIterator = None,
                 validation_iterator_aux: DataIterator = None,
                 shuffle: bool = True,
                 num_epochs: int = 20,
        ...
        self.iterator_aux = iterator_aux
        self._validation_iterator = validation_iterator
        self._validation_iterator_aux = validation_iterator_aux
        self._mixing_ratio = mixing_ratio
        self._cutoff_epoch = cutoff_epoch
        self.shuffle = shuffle
        self.optimizer = optimizer
        self.train_data = train_dataset
        self.train_data_aux = train_dataset_aux
    ...
    def batch_loss(self, batch: torch.Tensor, for_training: bool, batch_aux: torch.Tensor=None) -> torch.Tensor:
        """
        ...
        if batch_aux is not None:
            if self._multiple_gpu:
                output_dict_aux = self._data_parallel(batch_aux)
            else:
                batch_aux = util.move_to_device(batch_aux, self._cuda_devices[0])
                output_dict_aux = self.model(**batch_aux)
            try:
                loss_aux = output_dict_aux["loss"]
                if for_training:
                    loss_aux += self.model.get_regularization_penalty()
            except KeyError:
    ...
    def _create_generators(self):
        ...
        self.train_generator = self.iterator(self.train_data,
                                        num_epochs=self.train_num_epochs,
                                        shuffle=self.shuffle,
                                        remainder=self.train_remainder,
                                        num_batches=self.num_batches_train)
        self.train_generator_aux = self.iterator_aux(self.train_data_aux,
                                        num_epochs=self.train_aux_num_epochs,
                                        shuffle=self.shuffle,
                                        remainder=self.train_aux_remainder,
                                        num_batches=self.num_batches_train_aux)
        self.train_generator_tqdm = Tqdm.tqdm(self.train_generator,
                                         total=self.num_training_batches)#-max(self.train_num_epochs, self.train_aux_num_epochs))
        """
        Notes from jkasai
        - # of small epochs because the last one doesn't contribute to the tqdm counter.
        """
    def _prepare_iteration(self):
        if type(self.train_data) == type(()) :
            num_samples = self.train_data[1]
            self.train_data = self.train_data[0]
        else:
            num_samples = self.train_data.num_samples
        ## This is a bit silly, but poly_bucket does tiling and sampling in the iterator, which changes the number of batches.
        ## Only poly_bucket has the num_batches attribute
        num_batches_train = math.ceil(num_samples / self.iterator._batch_size)
        if type(self.train_data_aux) == type(()) :
            num_samples_aux = self.train_data_aux[1]
            self.train_data_aux = self.train_data_aux[0]
        else:
            num_samples_aux = self.train_data_aux.num_samples
        num_batches_train_aux = math.ceil(num_samples_aux / self.iterator_aux._batch_size)

        # Get tqdm for the training batches
        if num_batches_train > num_batches_train_aux:
            ## Primary training is bigger
            self.train_num_epochs = 1
            self.train_remainder = 0
            self.num_training_batches = num_batches_train
            self.train_aux_num_epochs = num_batches_train//num_batches_train_aux
            self.train_aux_remainder = num_batches_train - num_batches_train_aux*self.train_aux_num_epochs
        else:
            ## Auxiliary training is bigger
            self.train_aux_num_epochs = 1
            self.train_aux_remainder = 0
            self.num_training_batches = num_batches_train_aux
            self.train_num_epochs = num_batches_train_aux//num_batches_train
            self.train_remainder = num_batches_train_aux - num_batches_train*self.train_num_epochs
        self.num_batches_train = num_batches_train
        self.num_batches_train_aux = num_batches_train_aux
        if type(self._validation_data) == type(()) :
            num_val_samples = self._validation_data[1]
            self._validation_data = self._validation_data[0]
        else:
            num_val_samples = self._validation_data.num_samples
        self.num_validation_batches = math.ceil(num_val_samples / self.iterator._batch_size)
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        logger.info("Training")
        for batch, batch_aux in zip(self.train_generator_tqdm, self.train_generator_aux):
            ...
            metrics = self._get_metrics(train_loss, self.batches_this_epoch)
            description = self._description_from_metrics(metrics)

            self.train_generator_tqdm.set_description(description, refresh=False)
            ...
            if (self.num_batches_train < self.num_training_batches) and (self.batches_this_epoch % (self.num_batches_train*1) == 0):
                ## end of a small epoch if primary training is smaller
                self.train_generator_tqdm.update(1)
                return self._get_metrics(train_loss, self.batches_this_epoch, reset=True), False
            if (self.num_batches_train_aux < self.num_training_batches) and (self.batches_this_epoch % (self.num_batches_train_aux*1) == 0):
                ## end of a small epoch if auxiliary training is smaller
                self.train_generator_tqdm.update(1)
                return self._get_metrics(train_loss, self.batches_this_epoch, reset=True), False
        return self._get_metrics(train_loss, self.batches_this_epoch, reset=True), True
    ...
    def _validation_loss(self) -> Tuple[float, int]:
        ...
        val_generator_tqdm = Tqdm.tqdm(val_generator,
                                       total=self.num_validation_batches)
    ...
    def train(self) -> Dict[str, Any]:
        ...
        """
        try:
            epoch_counter, self.validation_metric_per_epoch = self._restore_checkpoint()
        except RuntimeError:
            traceback.print_exc()
        ...
        #num_batches_train, num_batches_aux = self._precompute_for_sampling()
        self._prepare_iteration()
        renew_generators = True
        metrics: Dict[str, Any] = {}
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            if renew_generators:
                ...
                print('\n')
                print('Renewing generators')
            train_metrics, renew_generators = self._train_epoch(epoch)

            metrics = self._validation(train_metrics, metrics, epoch, epochs_trained, epoch_counter)
            epochs_trained += 1
            ...
            if self._validation_data is not None:
                if self._should_stop_early(self.validation_metric_per_epoch):
                    logger.info("Ran out of patience.  Stopping training.")
                    break

        return metrics
    def _validation(self, train_metrics, metrics, epoch, epochs_trained, epoch_counter, prefix = 'metrics_epoch'):
        if self._validation_data is not None:
            with torch.no_grad():
                ...
                is_best_so_far = self._is_best_so_far(this_epoch_val_metric, self.validation_metric_per_epoch)
                self.validation_metric_per_epoch.append(this_epoch_val_metric)
        ...
        if self._serialization_dir:
            dump_metrics(os.path.join(self._serialization_dir, '{}_{}.json'.format(prefix, metrics['epoch'])), metrics)
        ...
        self._save_checkpoint(epoch, self.validation_metric_per_epoch, is_best=is_best_so_far)
    ...
    def from_params(cls,  # type: ignore
                    ...
                    params: Params,
                    validation_iterator: DataIterator = None,
                    validation_iterator_aux: DataIterator = None) -> 'Trainer':
        ...
        return cls(model, optimizer, iterator, iterator_aux,
                   train_data, train_data_aux, mixing_ratio, cutoff_epoch, validation_data, validation_data_aux,
                   patience=patience,
...
MultitaskTrainer.register("default")(MultitaskTrainer)
