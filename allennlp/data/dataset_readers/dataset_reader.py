...
class _LazyInstances(Iterable):
    ...
    """
    def __init__(self, instance_generator: Callable[[], Iterator[Instance]], num_samples: int) -> None:
        super().__init__()
...
class DatasetReader(Registrable):
    ...
    def read(self, file_path: str, get_num_samples: bool = False) -> Iterable[Instance]:
        """
        ...
        if lazy:
            num_samples = self._get_num_samples(file_path)
            return _LazyInstances(lambda: iter(self._read(file_path)), num_samples)
        else:
