...
def sort_by_padding(instances: List[Instance],
    ...
    #print('train size')
    #print(len(instances_with_lengths))
    return [instance_with_lengths[-1] for instance_with_lengths in instances_with_lengths]

def sort_by_lang(instances: List[Instance]):
    instances_by_lang = defaultdict(list)
    for instance in instances:
        instances_by_lang[instance.fields['metadata'].metadata['lang']].append(instance)
    return instances_by_lang


@DataIterator.register("poly_bucket")
class PolyBucketIterator(DataIterator):
    """
...
r       For example, ``[("sentence1", "num_tokens"), ("sentence2", "num_tokens"), ("sentence1",
    ...
    @overrides
    def _create_batches(self, instances: Iterable[Instance], shuffle: bool, sorting: bool=True, cutoff: bool=False) -> Iterable[Batch]:
        for instance_list in self._memory_sized_lists(instances):
#            if cutoff:
#                max_sent_len = 210
#                instance_list = [instance for instance in instance_list if len(instance.fields['metadata']['words'])<=max_sent_len]
            instances_by_lang = sort_by_lang(instance_list)
            max_lang = max(instances_by_lang.keys(), key=lambda lang: len(instances_by_lang[lang]))
            max_size = len(instances_by_lang[max_lang])
            instance_list_langs = []
            for lang, instances in instances_by_lang.items():
                ...
                if sorting:
                    instances = sort_by_padding(instances,
                                                ...
                                                self._padding_noise)
                #print('len')
                #print(lang, len(instances))
                if max_size != lang_size:
                    factor = max_size//lang_size
                    instances = instances*factor ## tile
                    remain = max_size-factor*lang_size
                    if remain > 0:
                        #instances += list(np.random.choice(instances[:lang_size], remain, replace=False)) ## sample the remaining ## choice doesn't work any more. it's a list of lists
                        idxes = np.random.randint(0, lang_size, [remain])
                        for idx in idxes:
                            instances.append(instances[idx])
                instance_list_langs.append(lazy_groups_of(iter(instances), self._batch_size//len(instances_by_lang)))

            batches = []
            #print('batch length')
            #print(len(list(zip(*instance_list_langs))))
            for batch_instances in zip(*instance_list_langs):
                batch_instances = [item for sublist in batch_instances for item in sublist]
                for possibly_smaller_batches in self._ensure_batch_is_sufficiently_small(batch_instances):
                    batches.append(Batch(possibly_smaller_batches))

            #print('print batches')
            #print(str(batches[0].instances[0]))
