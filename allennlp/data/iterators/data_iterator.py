...
class DataIterator(Registrable):
    ...
    def __call__(self,
                 ...
                 shuffle: bool = True,
                 remainder: int = 0,
                 num_batches: int = None) -> Iterator[TensorDict]:
        """
        ...
        """
        Note from jkasai
        remainder for multitask over-sampling
        """

        if num_epochs is None:
            epochs: Iterable[int] = itertools.count(starting_epoch + 1)
            final_epoch = starting_epoch
        else:
            epochs = range(starting_epoch, starting_epoch + num_epochs + 1) ## + 1 for remainder
            final_epoch = starting_epoch + num_epochs

        for epoch in epochs:
            self._epochs[key] = epoch
            if epoch == final_epoch:
                ## Last epoch. Only partial tiling by sampling.
                final = True
                if remainder == 0:
                    break
                logger.info("Last epoch. Partial tiling of {} samples from {} batches".format(remainder, num_batches))
                sample_idxes = list(np.random.choice(num_batches, size=remainder, replace=False))
            ...
            if self._cache_instances and key in self._cache:
                ...
                for batch in batches:
                    if final and (batch_idx not in sample_idxes):
                        batch_idx += 1
                        continue
                    if self._track_epoch:
                    ...
                    batch_idx += 1
                    yield tensor_dict
    def feed_predictions(self,
                 instances: Iterable[Instance],
                 num_epochs: int = None,
                 shuffle: bool = False,
                 sorting: bool = True) -> Iterator[Batch]:
        key = id(instances)
        starting_epoch = self._epochs[key]
        ...
        for epoch in epochs:
            ...
            batches = self._create_batches(instances, shuffle, sorting)#, cutoff=False)
            yield from batches
    ...
    def get_num_batches(self, instances: Iterable[Instance]) -> int:
        ...
        if is_lazy(instances) and self._instances_per_epoch is None:
            ...
            #print('not lazy')
            #print(len(ensure_list(instances)))
            #print(math.ceil(len(ensure_list(instances)) / self._batch_size))
