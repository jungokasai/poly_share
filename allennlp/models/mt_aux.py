...
class MTAux(Model):
    ...
    def __init__(self,
        ...
        super(MTAux, self).__init__(vocab)
        ...
        if use_bleu:
            ...
            for idx, token in self.vocab.get_index_to_token_vocabulary(self._target_namespace).items():
                if  token[-7:] == '@start@':
        ...
        self._intermediate_projection_layer = Linear(self._decoder_output_dim, projection_dim)
        self._output_projection_layer = Linear(projection_dim, num_classes)
        """
        Notes from jkasai
        add projection to save memory
        """
    ...
    def forward(self,  # type: ignore
        ...
        """
        state = self._init_encoded_state(embedded_text, source_mask)
        ...
        if target_tokens:
            ...
            output_dict = {}
        self.training = False
        if not self.training and self._beam_search:
    ...
    def _init_encoded_state(self, embedded_input: torch.Tensor, source_mask: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        ...
        """
        In the auxiliary model takes encoded_text directly from a primary task. No need to embed, but we need to run a new encoder (possibly shallow like one layer LSTM) for MT.
        jkasai
        """
    ...
    def _forward_loop(self,
                      ...
                      lang: str = 'eng') -> Dict[str, torch.Tensor]:
        """
        ...
        # shape: (batch_size,)
        if target_tokens:
            last_predictions = target_tokens['tokens'][:,0]
        else:
            last_predictions = source_mask.new_full((batch_size,), fill_value=self.vocab.get_token_index(lang+':'+START_SYMBOL, self._target_namespace))
        """
        Notes from jkasai
        start_index is target language-specific now.
        """
    ...
    def _prepare_output_projections(self,
        ...
        # shape: (group_size, num_classes)
        output_projections = self._intermediate_projection_layer(decoder_hidden)
        output_projections = self._output_projection_layer(output_projections)
