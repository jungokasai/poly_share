...
class BiaffineDependencyParser(Model):
    ...
    def __init__(self,
        ...
        #pos_tag_embedding = None ## for multitasking, let's assume no POS tag input for now.
    ...
    """
    get_encoded_text + get_output_dict does forward, but we need this decomposition for multitasking factorization
    """
    def get_encoded_text(self,  # type: ignore
                words: Dict[str, torch.LongTensor],
                pos_tags: torch.LongTensor=None) -> (torch.Tensor, torch.Tensor):
        # pylint: disable=arguments-differ
        embedded_text_input = self.text_field_embedder(words)
        batch_size, sequence_length, embedding_size = embedded_text_input.size()

        input_list = [embedded_text_input]
        if pos_tags is not None and self._pos_tag_embedding is not None:
            embedded_pos_tags = self._pos_tag_embedding(pos_tags)
            input_list.append(embedded_pos_tags)
        elif self._pos_tag_embedding is not None:
            raise ConfigurationError("Model uses a POS embedding, but no POS tags were passed.")
        #if self.langid_embedding is not None: 
        #    embedded_langids = self.langid_embedding(langid.long())
        #    ## [batch_size, langid_dim]
        #    embedded_langids = embedded_langids.unsqueeze(1).repeat(1, sequence_length, 1)
        #    input_list.append(embedded_langids)
        embedded_text_input = torch.cat(input_list, -1)
    ...
    def get_output_dict(self,  # type: ignore
