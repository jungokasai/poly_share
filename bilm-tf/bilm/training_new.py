...
from .data import Vocabulary, UnicodeCharsVocabulary, PolyglotCharsVocabulary, InvalidNumberOfCharacters
        ...
        with tf.device("/cpu:0"):
            if 'file' in self.options['word_emb']:
                ...
                npembed =  load_word_embedding(self.options['word_emb']['file'],
                                         ...
                                         output_dim)
                self.embedding_weights = tf.get_variable(name="embedding", 
...
def train(options, data, dev_data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        if restart_ckpt_file is not None:
            ...
            loader = tf.train.import_meta_graph(restart_ckpt_file + '.meta')
        ...
        if dev_data is not None:
            dev_data_gen = data.iter_batches(batch_size * n_gpus, unroll_steps)
            best_dev_plx = 100000000.0
        patience = 0
        for batch_no, batch in enumerate(data_gen, start=1):
                ...
                try:
                    if 'tokens_characters' in keyn.name and 'reverse' not in keyn.name:
                        ...
                        if x.max() > data.vocab._data_forward.vocab.n_chars:
                            print("Weird character id in feed_dict")
                            ipy.embed()
                except AttributeError:
                    continue

            # embed here to examine feed_dict
            # e.g. check next_token_id for unks
            ...
            if (batch_no % 1250 == 0) and dev_data is not None:
                # evaluate against dev data

                tot_dev_plx = 0
                for i, dev_X in enumerate(dev_data_gen):
                    dev_feed_dict = {t: v for t, v in zip(
                        init_state_tensors, init_state_values)}
                    for k in range(n_gpus):
                        model = models[k]
                        start = k * batch_size
                    ...
                    dev_summary, dev_plx, init_state_values = sess.run(
                        [summary_op, train_perplexity] + final_state_tensors, 
                        feed_dict=dev_feed_dict)
                    
                    tot_dev_plx += dev_plx
                    
                avg_dev_plx = tot_dev_plx/(i+1)
                if avg_dev_plx < best_dev_plx:
                    best_dev_plx = dev_plx
                    patience = 0
                else:
                    patience += 1

            # break if no improvement in 10 epochs (=12500 batchs) or something like that
            # maybe check less often than every 1250 epochs?
            if patience > 10:
                # not improving on dev data; stop training
                break
...
def load_vocab(vocab_filepath, max_word_length=None, polyglot=False):
    if polyglot:
        if max_word_length is None:
            raise ValueError(("Cannot use polyglot model without character "
                             "representation (yet) but max_word_length is not set"))
        return PolyglotCharsVocabulary(vocab_filepath, max_word_length,
                                       validate_file=True)
    elif max_word_length:
        return UnicodeCharsVocabulary(vocab_filepath, max_word_length,
                                      validate_file=True)
    else:
        return Vocabulary(vocab_filepath, validate_file=True)
